...
namespace EasyModbus
{
	...
	public class ModbusClient
	{
        ...
        {
        	long ticksWait = ((40*10000000) / this.baudRate);
            ...
            while ((DateTime.Now.Ticks - dateTimeLastRead.Ticks) < ticksWait) ; 						//10.000 Ticks in 1 ms
            dataReceived = true;
            receiveActive = false;
		...
		public bool[] ReadDiscreteInputs(int startingAddress, int quantity)
		{
            ...
            if (serialport != null)
            {
                ...
                while (receivedUnitIdentifier != this.unitIdentifier & !((DateTime.Now.Ticks - dateTimeSend.Ticks) > 1000000))
                {
                	while (dataReceived == false & !((DateTime.Now.Ticks - dateTimeSend.Ticks) > 1000000))
                    	System.Threading.Thread.Sleep(1);       
                	data = new byte[2100];
		...
		public bool[] ReadCoils(int startingAddress, int quantity)
		{
           ...
           data[12] = crc[0];
            ...
            if (serialport != null)
            {
                ...
                DateTime dateTimeSend = DateTime.Now;
                byte receivedUnitIdentifier = 0;
                while (receivedUnitIdentifier != this.unitIdentifier & !((DateTime.Now.Ticks - dateTimeSend.Ticks) > 10000000))
                {
                	while (dataReceived == false & !((DateTime.Now.Ticks - dateTimeSend.Ticks) > 10000000))
                    	System.Threading.Thread.Sleep(1);  
                	data = new byte[2100];
